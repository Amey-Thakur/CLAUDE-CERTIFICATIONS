...
- B. Confirm each named document exists and says what the summary claims
- C. Ask Claude to double-check its own work and circulate if it agrees
...
- B. "You are writing a weekly update for non-technical executives. From the log, produce: status in one line, three progress items, two risks with owners, and next week's focus. Keep it under 200 words."
- C. "Write something good about this project log for my boss."
- D. "Summarize this project log. Be detailed and thorough and professional."
...
- A. The most capable model, for consistency
- B. A fast, low-cost model, reserving a more capable one for exceptions
...
- B. Strip or pseudonymize the personal columns, then analyze what remains
- C. Upload it and ask Claude to disregard the personal columns
...
- B. State the format explicitly, including the number of bullets and their maximum length
- C. Switch to a more capable model
- D. Manually reformat each response

**7.** Which of these is the strongest signal that a Claude-generated claim needs verification before use?

- A. The answer is long
- B. The claim is a specific number, date, or citation that would matter if wrong
...
- B. The house tone, required disclaimers, and the standard structure of a client message
- C. Today's date
...
- B. Omission, which requires reading the source rather than only reviewing the output
...
- A. Whether the model changed
- B. Whether the new documents are diluting or contradicting the sources the answers should use
...
- B. Verifying the claims that would cause harm if wrong, then having a second person review customer-facing material
- C. Sending it with a note that AI assisted
...
- B. "Here are the current approval steps and where delays occur. Propose two redesigns: one that keeps the current structure and one that reorders or removes steps, with the tradeoffs of each."
- C. "Automate our approvals."
- D. "What do other companies do for approvals?"

**15.** Which situation most clearly calls for escalating to a technical colleague rather than solving it yourself?

- A. A prompt that needs better structure
- B. A request to connect Claude directly to the company's internal order database through an integration
...
| # | Answer | Domain | Why |
| --- | --- | --- | --- |
| 1 | B | Output Evaluation and Validation | Named sources can be fabricated; verification against the actual documents is the diligence step. Self-checking (C) is not independent evidence, and a disclaimer (D) does not make content correct |
| 2 | B | Prompting and Task Execution | Role, audience, explicit structure, and length constraints are what turn a vague request into a usable draft. Intensifiers like "detailed and thorough" (D) add no structure |
| 3 | B | Product and Model Selection | Match the model tier to the work: fast and cheap for formulaic volume, capable for exceptions |
...
| 5 | B | Governance, Risk, and Responsible Use | Make the task compliant, then do it. A requester's seniority does not override policy, and instructing the model to ignore data does not un-send it |
| 6 | B | Prompting and Task Execution | Format failures are fixed by stating the format, not by re-rolling or upgrading the model |
| 7 | B | Output Evaluation and Validation | Specific, consequential details are the highest-risk surface for fabrication. Length, speed, and vocabulary carry no accuracy signal |
...
| 9 | B | Configuration and Knowledge Management | Durable, shared conventions belong in instructions; per-message specifics belong in the prompt |
| 10 | B | Output Evaluation and Validation | Omission is as damaging as fabrication and is only visible if you read the source, not just the output |
| 11 | B | Troubleshooting and Optimization | Diagnose the change that coincided with the regression before rewriting anything |
...
| 13 | B | Output Evaluation and Validation | Verification of consequential claims plus human review for customer-facing material. Model size is not a control |
| 14 | B | Workflow Integration and Solution Design | Process redesign requires giving Claude the current steps, the pain points, and asking for alternatives with tradeoffs |
| 15 | B | Governance, Risk, and Responsible Use | System integrations belong to the Developer and Architect scope; recognizing that boundary is an Associate competency |
